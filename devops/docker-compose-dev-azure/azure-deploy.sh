...
resourcename="foo"  # lower case, only
resourcegroup="foo_rg"
...
# git clone https://github.com/apilogicserver/foo.git
# cd foo
...
az group create --name foo_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group foo_rg --sku S1 --is-linux
az container create --resource-group foo_rg --name foo --image apilogicserver/foo:latest --dns-name-label foo --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//foo.westus.azurecontainer.io:5656'
